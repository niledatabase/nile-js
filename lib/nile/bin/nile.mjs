...
if (!rootDir.endsWith("/lib/nile")) {
  rootDir = path.join(process.cwd(), "node_modules/@niledatabase/js");
}
...
prog
  ...
  .option(
    "-a, --api",
    "the FQDN of the nile instance eg. https://api.thenile.dev"
  )
...
async function rebuild(opts, token, password) {
  ...
  const url = opts?.api || opts?.a || "https://api.thenile.dev";
  ...
  if (verbose) {
    console.log("running yarn install in", rootDir);
  }

  exec("yarn install", { cwd: rootDir }, async (err, stdout) => {
    if (verbose) {
      console.log(stdout);
    }
